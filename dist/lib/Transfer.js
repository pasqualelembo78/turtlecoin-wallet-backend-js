...
function sendTransactionBasic(config, daemon, subWallets, destination, amount, paymentID, relayToNetwork, sendAll) {
    return __awaiter(this, void 0, void 0, function* () {
        return sendTransactionAdvanced(config, daemon, subWallets, [[destination, amount]], undefined, undefined, paymentID, undefined, undefined, relayToNetwork, sendAll);
    });
}
...
function sendTransactionAdvanced(config, daemon, subWallets, addressesAndAmounts, mixin, fee, paymentID, subWalletsToTakeFrom, changeAddress, relayToNetwork, sendAll, extraData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let [address] of addressesAndAmounts) {
            const decoded = yield turtlecoin_utils_1.Address.fromAddress(address, config.addressPrefix);
            ...
            if (decoded.paymentId !== '') {
                ...
                address = yield (yield turtlecoin_utils_1.Address.fromPublicKeys(decoded.spend.publicKey, decoded.view.publicKey, undefined, config.addressPrefix)).address();
        ...
        for (const [i, input] of availableInputs.entries()) {
            ...
            if (sumOfInputs >= totalAmount || sendAll) {
                ...
                if (fee.isFeePerByte) {
                    ...
                    const minFee = Utilities_1.getMinimumTransactionFee(tx.size, daemon.getNetworkBlockCount(), config);
...
function setupDestinations(addressesAndAmountsTmp, changeRequired, changeAddress, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const [address, amount] of amounts) {
            result.push({
                amount: amount,
                destination: yield turtlecoin_utils_1.Address.fromAddress(address, config.addressPrefix)
            });
        }
        return result;
    });
...
function makeTransaction(mixin, fee, paymentID, ourInputs, destinations, subWallets, daemon, config, extraData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const ourOutputs = yield Promise.all(ourInputs.map((input) => __awaiter(this, void 0, void 0, function* () {
            if (!input.input.privateEphemeral || !Utilities_1.isHex64(input.input.privateEphemeral)) {
                const [, tmpSecretKey] = yield CryptoWrapper_1.generateKeyImage(input.input.transactionPublicKey, subWallets.getPrivateViewKey(), input.publicSpendKey, input.privateSpendKey, input.input.transactionIndex, config);
        ...
        try {
            ...
            const tx = yield CnUtils_1.CryptoUtils(config).createTransaction(destinations, ourOutputs, randomOuts, mixin, fee, paymentID, undefined, extraData);
...
function relayTransaction(tx, fee, paymentID, inputs, changeAddress, changeRequired, subWallets, daemon, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const error = yield daemon.sendTransaction(tx.toString());
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            Logger_1.logger.log(`Failed to relay transaction. ${error}`, Logger_1.LogLevel.DEBUG, Logger_1.LogCategory.TRANSACTIONS);
        ...
        for (const [outputIndex, output] of keyOutputs.entries()) {
            if (output.type === turtlecoin_utils_1.TransactionOutputs.OutputType.KEY) {
                ...
                /* Derive the spend key from the transaction, using the previous
                   derivation */
                const derivedSpendKey = yield CryptoWrapper_1.underivePublicKey(derivation, outputIndex, o.key, config);
                /* See if the derived spend key matches any of our spend keys */
                if (!_.includes(spendKeys, derivedSpendKey)) {
                    continue;
                }
                const input = new Types_1.UnconfirmedInput(o.amount.toJSNumber(), o.key, hash);
...
function validateTransaction(destinations, mixin, fee, paymentID, subWalletsToTakeFrom, changeAddress, sendAll, currentHeight, subWallets, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let error = yield ValidateParameters_1.validateDestinations(destinations, config);
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        /* Validate stored payment ID's in integrated addresses don't conflict */
        error = yield ValidateParameters_1.validateIntegratedAddresses(destinations, paymentID, config);
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        /* Verify the subwallets to take from exist */
        error = yield ValidateParameters_1.validateOurAddresses(subWalletsToTakeFrom, subWallets, config);
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        /* Verify we have enough money for the transaction */
        error = yield ValidateParameters_1.validateAmount(destinations, fee, subWalletsToTakeFrom, subWallets, currentHeight, config);
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        ...
        error = yield ValidateParameters_1.validateOurAddresses([changeAddress], subWallets, config);
        if (!_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        return WalletError_1.SUCCESS;
...
function validateFusionTransaction(mixin, subWalletsToTakeFrom, destination, currentHeight, subWallets, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        error = yield ValidateParameters_1.validateOurAddresses([destination], subWallets, config);
        if (_.isEqual(error, WalletError_1.SUCCESS)) {
            return error;
        }
        return WalletError_1.SUCCESS;
